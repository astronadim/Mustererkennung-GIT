...
gt_label=unique(gt_(index));
for j=1:length(gt_label)
count_gt_label(j)=length(find(gt_(index)==gt_label(j)));
end
[max_count_gt_label,index_max_count_gt_label]=max(count_gt_label);
segment_gt_label=index_max_count_gt_label;
